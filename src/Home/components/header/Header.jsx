...
function Header() {
    ...
    useEffect(() => {
        ...
        headerRef.current.addEventListener("mousemove", (e)=>{
            let x = e.pageX + "px"
            let y =( e.pageY + 25) + "px"
            fallowMouse.current.style.left = x
            fallowMouse.current.style.top = y
        })
        // leave hover haeder
        headerRef.current.addEventListener("mouseleave", ()=>{
            fallowMouse.current.style.left = "50%"
            fallowMouse.current.style.top = "50%"
        })
    }, []);
    return (
        <>
            <header className={StatusVideo?"active":""} ref={headerRef} onClick={()=>setStatusVideo(!StatusVideo)}>
                <VideoHandler StatusVideo={StatusVideo} fallowMouse={fallowMouse}/>
